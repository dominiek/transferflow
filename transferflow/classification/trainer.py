#
# Modified from Tensorflow flower retrain example
#
...
def create_base_graph(model_filename, base_model_id):
  ...
  with tf.Session() as sess:
    with gfile.FastGFile(model_filename, 'rb') as f:
      ...
      bottleneck_tensor, jpeg_data_tensor, resized_input_tensor = (
          tf.import_graph_def(graph_def, name=base_model_id, return_elements=[
...
def train(scaffold_dir, base_graph_path, output_model_path, options={}):
    settings = DEFAULT_SETTINGS
    for key in options:
        settings[key] = options[key]

    image_dir = scaffold_dir + '/images'
    bottleneck_dir = scaffold_dir + '/bottlenecks'

    scaffold_model_id = ''
    final_tensor_name = 'retrained_layer'
    graph, bottleneck_tensor, jpeg_data_tensor, resized_image_tensor = (create_base_graph(base_graph_path, scaffold_model_id))
    image_lists = create_image_lists(image_dir, settings['testing_percentage'], settings['validation_percentage'])
    class_count = len(image_lists.keys())
    ...
    do_distort_images = should_distort_images(settings['flip_left_right'], settings['random_crop'], settings['random_scale'], settings['random_brightness'])
    sess = tf.Session()

    if do_distort_images:
        logger.info('Distorting images')
        distorted_jpeg_data_tensor, distorted_image_tensor = add_input_distortions(settings['flip_left_right'], settings['random_crop'], settings['random_scale'], settings['random_brightness'])
    else:
    ...
    for i in range(settings['num_steps']):
        ...
        if do_distort_images:
            train_bottlenecks, train_ground_truth = get_random_distorted_bottlenecks(
                sess, image_lists, settings['train_batch_size'], 'training',
        ...
        validation_bottlenecks, validation_ground_truth = (
            get_random_cached_bottlenecks(
                sess, image_lists, settings['validation_batch_size'], 'validation',
                bottleneck_dir, image_dir, jpeg_data_tensor,
                bottleneck_tensor))
    ...
    test_bottlenecks, test_ground_truth = get_random_cached_bottlenecks(
        sess, image_lists, settings['test_batch_size'], 'testing',
        bottleneck_dir, image_dir, jpeg_data_tensor,
        bottleneck_tensor)
    ...
    return image_lists, benchmark_info
